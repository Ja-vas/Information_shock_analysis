...
def process_car_quartiles():
    print("1. Loading raw CARs and Earnings...")
    car_df = pd.read_csv(Path(MAIN_DIR) / "car_results" / "CAR_raw_results.csv", low_memory=False)
    car_df['Date'] = pd.to_datetime(car_df['Date']).dt.date

    # =========================================================================
    # THE FIX: WINSORIZATION (Caps the extreme 1% outliers)
    # =========================================================================
    ...
    for col in car_cols:
        if col in car_df.columns:
            # Find the mathematical boundary for the 0.5th and 99.5th percentile
            lower_bound = car_df[col].quantile(0.005)
            upper_bound = car_df[col].quantile(0.995)
            
            # Clip (cap) any value that goes beyond these boundaries. 
            car_df[col] = car_df[col].clip(lower=lower_bound, upper=upper_bound)
    # =========================================================================

    # Load Earnings
    earn_df = pd.read_csv(Path(MAIN_DIR) / "gap_earnings_joined.csv", low_memory=False)
    ...
    # 3. Define the exact columns to calculate
    ...
    for direction in directions:
            ...
            for metric in METRICS:
                ...
                for q, group in grouped:
                    ...
                    for w in WINDOWS:
                        ...
                        # Because of the winsorization above, this mean is now safe and realistic!
        ...
        q_path = Path(MAIN_DIR) / f'CAR_quartiles_results_{direction}.csv'
